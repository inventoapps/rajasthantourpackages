...
export async function getSiteSettings() {
  ...
  try {
    ...
    // If PostgREST cache is stale, new columns will be missing — detect and fix
    if (data && typeof data.business_phone === 'undefined') {
      try {
        ...
        if (sqlData && Array.isArray(sqlData) && sqlData.length > 0) {
          return { ...DEFAULT_SITE_SETTINGS, ...sqlData[0] };
        }
        if (sqlData && typeof sqlData === 'object' && !Array.isArray(sqlData)) {
          return { ...DEFAULT_SITE_SETTINGS, ...sqlData };
        }
      } catch {
        // exec_sql not available — fall through to merge
      }
    }
